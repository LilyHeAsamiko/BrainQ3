function Features = features1(reconstructions, zef, tt, n, iter)
...
%     A3 = bsxfun(@rdivide, bsxfun(@times, A3, Y3), X3);    
    SX1 = SX1.*(A1'*SY + mu*W1.*SX1)./(A1'*SY.*SX1 +lambda*SX1.^(-0.5)+mu*W1.*SX1);
...
%Features = [x, y, z, SX(1:S/3,:), SX(S/3+1:2*S/3,:), SX(2*S/3+1:S, SX3, :)];
Features = [x, y, z, SX1(1:S/3,:), SX1(S/3+1:2*S/3,:), SX1(2*S/3+1:S)];
end